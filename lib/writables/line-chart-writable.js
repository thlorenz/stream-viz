 ...
 *
 *    - clazz: 'line-chart'     - class attribute given to the canvas element
 *    
 *    - minValue: 0,                - specify to clamp the lower y-axis to a given value
 *    - maxValue: undefined,        - specify to clamp the upper y-axis to a given value
 *    - maxValueScale: 1.2,         - allows proportional padding to be added above the chart. for 10% padding, specify 1.1.
...
function LineChartWritable (el, opts) {
  ...
  this._canvas.setAttribute('class', this._chartOpts.clazz);
  el.appendChild(this._canvas);

  this._timeSeries = new TimeSeries();

  this._chart = new Chart(this._chartOpts);
 // this._chart = new Chart({maxValueScale:1.01,timestampFormatter:Chart.timeFormatter});
  this._chart.addTimeSeries(this._timeSeries, this._timeSeriesOpts);
  this._chart.streamTo(this._canvas, 0);
}
